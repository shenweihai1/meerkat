...
DpdkTransport::DpdkTransport(uint16_t sm_udp_port, uint8_t rpc_id,
  ...
  std::cout << "init a DpdkTransport while sm_udp_port: " << sm_udp_port << ", rpc_id: " << unsigned(rpc_id)  << std::endl;
  _unused(sm_udp_port);

  {
    ...
    if (g_dpdk_initialized) {
      ...
      std::cout << "address of rte_eal_init: " <<  (ERPC_LOG_LEVEL >= ERPC_LOG_LEVEL_INFO) << std::endl;
      const int rte_argc =
          static_cast<int>(sizeof(rte_argv) / sizeof(rte_argv[0])) - 1;
      int ret = rte_eal_init(rte_argc, const_cast<char **>(rte_argv));
      std::cout << "address of rte_eal_init: " << ret << " : " << (ret >=0) << std::endl;
    ...
    if (dpdk_proc_type_ == DpdkProcType::kSecondary) {
      ...
      if (!g_port_initialized[phy_port]) {
        std::cout << "setup_phy_port: " << phy_port << std::endl;
        g_port_initialized[phy_port] = true;
