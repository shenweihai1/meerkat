...
void server_func(erpc::Nexus *nexus) {
  ...
  std::cout << "phy_port is 0? " << (phy_port == 0) << ", numa_node: " << FLAGS_numa_node << std::endl;
...
void client_func(erpc::Nexus *nexus) {
  ...
  send_req(c);
  std::cout << "test_ms: " << FLAGS_test_ms << std::endl;
...
int main(int argc, char **argv) {
  printf("Latency: Welcome! spec: 1\n");
  signal(SIGINT, ctrl_c_handler);
  gflags::ParseCommandLineFlags(&argc, &argv, true);
  erpc::rt_assert(FLAGS_numa_node <= 1, "Invalid NUMA node");

  std::cout << "uri: " << erpc::get_uri_for_process(FLAGS_process_id) << std::endl;
